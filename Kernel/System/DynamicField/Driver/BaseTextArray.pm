...
package Kernel::System::DynamicField::Driver::BaseTextArray;
...
use parent qw(Kernel::System::DynamicField::Driver::BaseArray);
...
Kernel::System::DynamicField::Driver::BaseTextArray - sub module of
Kernel::System::DynamicField::Driver::TextArray
...
sub ValueGet {
    ...
    my @Values;
    for my $DFValueItem ( @{ $DFValue } ) {
        return if !IsHashRefWithData($DFValueItem);
        push @Values, $DFValueItem->{ValueText};
    }
...
sub ValueSet {
    ...
    if ( !IsArrayRefWithData( $Param{Value} ) ) {
        $Kernel::OM->Get('Kernel::System::Log')->Log(
            Priority => 'error',
            Message  => "The value '$Param{Value}' is not ref to array!",
        );
        return;
    }
    ...
    for my $ValueItem ( @{ $Param{Value} } ) {
        push @Values, { ValueText => $ValueItem };
    }
...
sub ValueValidate {
    ...
    if ( !IsArrayRefWithData( $Param{Value} ) && ref $Param{Value} ) {
        $Kernel::OM->Get('Kernel::System::Log')->Log(
            Priority => 'error',
...
=cut
        ...
        )
    {
        ...
        if ( !ref $Param{Value} && IsStringWithData( $Param{Value} ) ) {
            REGEXENTRY:
            ...
            my $Success;
            for my $ValueItem ( @{ $Param{Value} } ) {
                return if !$Self->ValueValidate( %Param, Value => $ValueItem );
            }
        }
    }
...
sub SearchSQLGet {
    ...
    my $SQL;
    if ( $Param{Operator} eq 'Contains' ) {
        if ( IsArrayRefWithData($Param{SearchTerm}) ) {
            $SQL = " $Param{TableAlias}.value_text $Operators{ $Param{Operator} } ";
            my $In = join ', ', map { "'$_'" } @{ $Param{SearchTerm} };
            $SQL .= "(" . $In . ")";
        }
        elsif ( !ref $Param{SearchTerm} ) {
            $SQL = "$Param{TableAlias}.value_text = '$Param{SearchTerm}'";
        }
        else {
            ; # maybe some log here!
        }
...
sub EditFieldValueGet {
    ...
    if ( IsHashRefWithData( $Param{Template} ) && defined $Param{Template}->{$FieldName} ) {
        @Values = $Param{Template}->{$FieldName};
    }
    ...
    {
        @Values = $Param{ParamObject}->GetArray( Param => $FieldName );
    }
...
sub SearchFieldRender {
    ...
    # check and set class if necessary
    my $FieldClass = 'DynamicFieldTextArray';
...
EOF
    ...
    if ( $Param{UseLabelHints} ) {
        $AdditionalText = Translatable('contains one of the string');
    }
